...
jq -e --slurpfile schema "$schema" '
  . as $c | $schema[0] as $s |
  ($s.const.schema_version == "ai-sdlc-contract/v1") and
  (keys | sort == $s.required_top_level) and
  (.schema_version == $s.const.schema_version) and
  ...
  ([.status] - $s.enum.status | length == 0) and
  ([.executor] - $s.enum.executor | length == 0) and
  ([.priority] - $s.enum.priority | length == 0) and
  ([.task_type] - $s.enum.task_type | length == 0) and
