...
if grep -Fq '/v1/responses' "$WORKFLOW"; then
  echo 'not ok - workflow calls the Responses API' >&2; exit 1
fi
echo 'ok - workflow does not call the Responses API'
pass=$((pass + 1))
if grep -Fq 'openai-responses-preflight' "$WORKFLOW"; then
  echo 'not ok - workflow creates Responses API preflight files' >&2; exit 1
fi
echo 'ok - workflow does not create Responses API preflight files'
pass=$((pass + 1))
