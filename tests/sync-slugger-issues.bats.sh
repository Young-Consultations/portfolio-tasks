...
make_issue(){ jq -n --argjson n "$1" --arg title "$2" --arg body "$3" --arg state "$4" --argjson labels "$5" --argjson assignees "${6:-[]}" --arg url "https://github.com/Young-Consultations/portfolio-tasks/issues/$1" '{number:$n,title:$title,body:$body,state:$state,labels:($labels|map({name:.})),assignees:($assignees|map({login:.})),html_url:$url}'; }
...
body_for(){ local n=$1 title=$2 body=$3 state=$4 source_repo=${5:-Young-Consultations/portfolio-tasks}; printf '%s\n\n---\n## Portfolio Task Metadata\n- Source repository: `%s`\n- Source issue: `#%s`\n- Source URL: `https://github.com/Young-Consultations/portfolio-tasks/issues/%s`\n- Source state: `%s`\n- Managed automatically: Yes\n<!-- portfolio-task-source: %s#%s -->' "$body" "$source_repo" "$n" "$n" "$state" "$source_repo" "$n"; }
task_body(){ body_for "$@"; }
...
case_ok "unchanged eligible issue produces no-op" bash -c 'd=$(setup_mock c5); b=$(task_body 1 T B open); write_common "$d" "$(make_issue 1 T B open "[\"chatgpt-task\"]")" "[$(make_target 9 "[PORTFOLIO-TASK #1] T" "$b" open "[\"portfolio-task\"]")]"; run_case c5 no-op "$d"'
...
case_ok "token values never appear in logs" bash -c 'd=$(setup_mock c17); write_common "$d" "$(make_issue 1 T B open "[\"chatgpt-task\"]")" "[]"; GH_TOKEN=SECRET_TOKEN run_case c17 create "$d" true; ! grep -n "SECRET_TOKEN" "$d"/* /tmp/out /tmp/err'

case_ok "target matching ignores copied source markers" bash -c 'd=$(setup_mock c18); real=$(body_for 1 T B open); impersonator=$(body_for 99 wrong "copied <!-- portfolio-task-source: Young-Consultations/portfolio-tasks#1 --> marker" open); write_common "$d" "$(make_issue 1 T B open "[\"chatgpt-task\"]")" "[$(make_target 8 "wrong" "$impersonator" open "[\"portfolio-task\"]"),$(make_target 9 "[PORTFOLIO-TASK #1] T" "$real" open "[\"portfolio-task\"]")]"; run_case c18 no-op "$d"; grep -Fq "Matching target issue number: \`9\`" "$d/summary.md"'
case_ok "target search failure aborts before writes" bash -c 'd=$(setup_mock c19); echo "$(make_issue 1 T B open "[\"chatgpt-task\"]")" > "$d/GET_repos_Young-Consultations_portfolio-tasks_issues_1.json"; GH_TOKEN=SECRET run_case c19 rc:1 "$d" false; [[ ! -f "$d/writes.log" ]]; grep -q "API failures: Could not search target issues" "$d/summary.md"'
case_ok "source marker comments are stripped from synchronized bodies" bash -c 'd=$(setup_mock c20); write_common "$d" "$(make_issue 1 T "copied <!-- portfolio-task-source: Young-Consultations/portfolio-tasks#2 --> marker" open "[\"chatgpt-task\"]")" "[]"; GH_TOKEN=SECRET run_case c20 create "$d" false; grep -q "removed portfolio-task-source marker" "$d/writes.log"; ! grep -q "portfolio-tasks#2" "$d/writes.log"'

case_ok "legacy namespace target marker is updated instead of duplicated" bash -c 'd=$(setup_mock c21); b=$(body_for 1 T old open mightyjoe909/portfolio-tasks); write_common "$d" "$(make_issue 1 T B open "[\"chatgpt-task\"]")" "[$(make_target 9 "[PORTFOLIO-TASK #1] T" "$b" open "[\"portfolio-task\"]")]"; GH_TOKEN=SECRET run_case c21 update "$d" false; grep -Fq "Matching target issue number: \`9\`" "$d/summary.md"; grep -Fq "portfolio-task-source: Young-Consultations/portfolio-tasks#1" "$d/writes.log"; ! grep -Fq "portfolio-task-source: mightyjoe909/portfolio-tasks#1" "$d/writes.log"'

case_ok "active automation and documentation omit obsolete namespace" bash -c '
  files=(".github/workflows/sync-slugger-issues.yml" "scripts/sync-slugger-issues.sh" "README.md")
  ! grep -En "mightyjoe909/(slugger)|repos/mightyjoe909|github.com/mightyjoe909|(^|[^A-Z_])SOURCE_REPO=\"mightyjoe909/portfolio-tasks\"|(^|[^A-Z_])TARGET_REPO=\"mightyjoe909/slugger\"" "${files[@]}"
'
