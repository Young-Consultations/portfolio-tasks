...
case_ok "token values never appear in logs" bash -c 'd=$(setup_mock c17); write_common "$d" "$(make_issue 1 T B open "[\"chatgpt-task\"]")" "[]"; GH_TOKEN=SECRET_TOKEN run_case c17 create "$d" true; ! rg -n "SECRET_TOKEN" "$d" /tmp/out /tmp/err'
...
case_ok "active automation and documentation omit obsolete namespace" bash -c '
  ...
  ! rg -n "mightyjoe909/(slugger)|repos/mightyjoe909|github.com/mightyjoe909|(^|[^A-Z_])SOURCE_REPO=\"mightyjoe909/portfolio-tasks\"|(^|[^A-Z_])TARGET_REPO=\"mightyjoe909/slugger\"" "${files[@]}"
'
