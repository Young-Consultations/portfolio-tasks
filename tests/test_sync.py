...
def test_create_skip_and_disable_actions() -> None:
    ...
    assert (
        SyncPlanner.plan(issue(labels=()), issue(number=9), True).action is SyncAction.DISABLE_SYNC
    )
...
def test_non_slugger_targets_are_skipped() -> None:
    for repository in (
        ...
        "Young-Consultations/sandbox",
        "Young-Consultations/unknown-target",
    ):
...
def test_non_slugger_targets_disable_and_close_existing_mirror() -> None:
    source = issue(
        body=SLUGGER_ISSUE_BODY.replace(
            "Young-Consultations/slugger", "Young-Consultations/consulting-playbook"
        )
    )
    target = issue(number=9, labels=("portfolio-task", "manual"))

    plan = SyncPlanner.plan(source, target)

    assert plan.action is SyncAction.DISABLE_SYNC
    assert plan.payload is not None
    assert plan.payload["state"] == "closed"
    assert plan.payload["labels"] == ["manual"]
    assert "Managed automatically: No - non-slugger target repository" in plan.payload["body"]
...
def test_close_reopen_and_noop() -> None:
    ...
    target = issue(
        number=9, title=desired["title"], body=desired["body"], labels=("portfolio-task",)
    )
...
def test_locator_uses_terminal_marker_and_migrates_legacy_namespace() -> None:
    ...
    copied = issue(
        number=8,
        body="x <!-- portfolio-task-source: Young-Consultations/portfolio-tasks#1 --> copied",
    )
