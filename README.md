...
The workflow `.github/workflows/sync-slugger-issues.yml` synchronizes qualifying issues from `mightyjoe909/portfolio-tasks` to `mightyjoe909/slugger`.
...
```html
<!-- portfolio-task-source: mightyjoe909/portfolio-tasks#<source-issue-number> -->
```
...
Source assignees are included in create or update payloads. If GitHub rejects an assignee because the user cannot be assigned in `mightyjoe909/slugger`, the workflow reports the failure without printing credentials.
...
- `mightyjoe909/portfolio-tasks`: Metadata read, Issues read.
- `mightyjoe909/slugger`: Metadata read, Issues read and write.

### Manual GitHub setup

1. Create the `chatgpt-task` label in `mightyjoe909/portfolio-tasks`.
2. Create the `portfolio-task` label in `mightyjoe909/slugger`; if it is missing, label application may be skipped or reported by GitHub.
3. Create a fine-grained personal access token or GitHub App token.
4. Limit token repository access to `mightyjoe909/portfolio-tasks` and `mightyjoe909/slugger`.
