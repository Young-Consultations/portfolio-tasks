...
| WA-11 | Current target activation is mutable `.github` control-plane state, separate from the pinned compatibility unit, and is enforced by the router before dispatch. | Targets must not enforce historical activation or treat capability registration as enablement; inactive and unknown selections fail closed without dispatch. | Organization owner; router activation and dispatch conformance evidence. |
